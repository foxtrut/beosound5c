...
# Schemes Sonos reports for raw radio streams. Sonos rewrites the URI we hand
# it (x-rincon-mp3radio://live1.sr.se/… comes back as aac://http://edge2.sr.se/…),
# so streams are recognized by scheme family, never by the URL we played.
RADIO_STREAM_SCHEMES = ("x-rincon-mp3radio://", "aac://", "hls-radio://",
                        "mp3radio://")
...
class MediaServer(PlayerBase):
    ...
    def __init__(self):
        ...
        # Last play command was a radio stream (see _is_radio_stream).
        self._radio_playback = False
    ...
    async def play(self, uri=None, url=None, track_uri=None, meta=None,
        ...
        """
        self._radio_playback = bool(radio)
        if radio:
    ...
    def _is_radio_stream(self, uri) -> bool:
        """True when *uri* is the stream from our own radio play command.

        The radio source owns that stream's router metadata (programme title
        + artwork); Sonos only sees the ICY title with no artwork, so every
        broadcast path checks this and stays silent.
        """
        return (self._radio_playback and isinstance(uri, str)
                and uri.startswith(RADIO_STREAM_SCHEMES))
        ...
        while self.running:
            try:
                ...
                try:
                    ...
                    # Skip for radio: _cached_media_data still holds the
                    # previous track, and the source broadcasts its own pause.
                    if (prev_state == 'playing' and state in ('paused', 'stopped')
                            and not self._is_radio_stream(self._current_track_id)):
                ...
                if track_info:
                    ...
                    # Non-stream content took over (e.g. Sonos app) — Sonos
                    # metadata ownership resumes. Empty uri is transient.
                    if (track_changed and track_id and self._radio_playback
                            and not self._is_radio_stream(track_id)):
                        logger.info("Radio stream replaced by other content")
                        self._radio_playback = False
                    ...
                    if track_changed or position_jumped:
                        ...
                        if self._is_radio_stream(track_id):
                            # Radio source owns metadata — don't broadcast and
                            # don't arm the retry (a recovered broadcast is how
                            # the ICY title used to clobber the SR artwork).
                            self._pending_broadcast = False
                            if track_changed:
                                logger.info("Radio stream track change — "
                                            "metadata left to the radio source")
                        elif suppress:
                        ...
                        if track_changed and not self._is_radio_stream(track_id):
                            self._spawn(
                    ...
                    # commands route directly to the player. A change TO our
                    # own radio stream is never external (slow stream start).
                    if (track_changed
                            and not self._is_radio_stream(track_id)
                            and self.seconds_since_command() > USER_ACTION_HORIZON):
                        self._spawn(
    ...
    async def fetch_media_data(self):
        ...
        try:
            ...
            # Our radio stream: the source owns router metadata — same
            # contract as the ZPSTR drop above, covering every call site.
            if self._is_radio_stream(track_info.get('uri', '')):
                logger.debug("Skipping media update (radio stream — "
                             "source owns metadata)")
                return None
